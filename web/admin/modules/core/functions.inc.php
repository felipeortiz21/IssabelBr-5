...
function core_getdestinfo($dest)
{
    ...
    if (substr(trim($dest), 0, 16) == 'from-did-direct,') {
        ...
        if (empty($thisexten)) {
            ...
            return array(
                'description' => sprintf(__("User Extension %s: %s"), $exten, $thisexten['name']),
        ...
        if (empty($thisexten)) {
            ...
            return array(
                'description' => sprintf(__('Trunk: %s (%s)'), $name, $thisexten['tech']),
        ...
        $display = ($amp_conf['AMPEXTENSIONS'] == "deviceanduser") ? 'users' : 'extensions';
        return array(
            'description' => 'User Extension ' . $exten . ': ' . $thisexten['name'],
            'edit_url' => "config.php?type=setup&display=$display&extdisplay=" . urlencode($exten) . "&skip=0",
        );
...
function core_users_add($vars, $editmode = false)
{
    ...
    // ===== LIMITE DINAMICO DE RAMAIS =====
    $sql_limite = "SELECT limite FROM ramais_limite WHERE ativo = 1 ORDER BY id DESC LIMIT 1";
    $res_limite = sql($sql_limite, "getRow", DB_FETCHMODE_ASSOC);

    if (!empty($res_limite)) {

        $limite_ramais = (int)$res_limite['limite'];

        $sql_total = "SELECT COUNT(*) as total FROM users";
        $res_total = sql($sql_total, "getRow", DB_FETCHMODE_ASSOC);

        if ($res_total['total'] >= $limite_ramais) {
            echo "<script>javascript:alert('Limite máximo de ramais atingido.');</script>";
            return false;
        }
    }
    // =====================================
