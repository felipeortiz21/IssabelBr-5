    ...
    if($uid===FALSE)
        ...
        if($val === TRUE){
            ...
            if($status){
                ...
                $arrResult['msg'] = _tr("Senha alterada com seucesso!.");
                $_SESSION['issabel_pass'] = md5($new_pass);
            }else{
                $arrResult['msg'] = _tr("Nao foi possivel alterar a senha.");
            }
        }else{
            $arrResult['msg'] = _tr("Nao foi possivel alterar a senha. A senha atual esta incorreta.");
        }
    }
    return $arrResult;
}
...
SQL_LEER_BOOKMARKS_ACTUALES;
    ...
    if (!$r) {
        ...
        $arrResult['msg'] = (
            $exist
                ? _tr('Bookmark cannot be removed. Please try again or contact with your Spiral administrator and notify the next error')
                : _tr('Bookmark cannot be added. Please try again or contact with your Spiral administrator and notify the next error')
            ).': '.$pdbACL->errMsg;
    } else {
        $pdbACL->commit();
        $arrResult['status'] = TRUE;
        ...
        if($uid!==FALSE){
            ...
            if($exist){
                ...
                if(!$r){
                    $pdbACL->rollBack();
                    $arrResult['status'] = FALSE;
                    $arrResult['msg'] = _tr("Request cannot be completed. Please try again or contact with your Spiral administrator and notify the next error: ").$pdbACL->errMsg;
                    return $arrResult;
                }else{
                    $pdbACL->commit();
                    $arrResult['status'] = TRUE;
