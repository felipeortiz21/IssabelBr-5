...
function _moduleContent(&$smarty, $module_name)
{
    ...
    if(is_array($arrResult['cdrs']) && $total>0) {
        foreach($arrResult['cdrs'] as $key => $value) {
            $arrTmp = array_fill(0, 16, ""); // Cria um array com 16 posições vazias (0 a 15)

            $arrTmp[0] = $value[0];  // Date
            $arrTmp[1] = $value[1];  // Source
            $arrTmp[2] = $value[11]; // Ring Group
            $arrTmp[3] = $value[2];  // Destination
            $arrTmp[4] = $value[3];  // Src. Channel
            $arrTmp[5] = $value[9];  // Account Code
            $arrTmp[6] = $value[4];  // Dst. Channel
            ...
            if ($value[5] == "ANSWERED") {
               ...
               $value[5] = "<font color=red>"._tr($value[5])."</font>";
                ...
                $value[5] = "<font color=orange>"._tr($value[5])."</font>";
            ...
            $arrTmp[8]  = $sTiempo;   // Duration
            $arrTmp[9]  = $value[6];    // UniqueID
            
            // Recording / CEL Icon
            if(!$disableCel) {
                $arrTmp[10] = '<a onclick="showCel(\''.$value[6].'\')"> <span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span> </a>';
            } else {
                $arrTmp[10] = ""; 
            }

            $arrTmp[11] = $value[2];  // Cnum (Ajuste conforme sua necessidade)
            $arrTmp[12] = "";         // Cnam (Vazio para evitar erro)
            $arrTmp[13] = "";         // Outbound Cnum
            $arrTmp[14] = $value[16]; // DID
            $arrTmp[15] = $value[17]; // User Field
